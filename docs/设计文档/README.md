...
| 调用方 | 被调用方 | 怎么调 | 什么时候 |
|--------|---------|--------|---------|
| Loader | Memory | `mem_map()` 分配区域，`mem_load()` 写入代码/数据 | 启动时 |
| CPU | Memory | `mmu_read_32(mmu, pmem, vaddr, ...)` 取指令 / 读写数据 | 每条指令的执行阶段 |
| Debugger | CPU | 读 `regs[]`、`PC`，控制 `running`、`single_step` | 用户命令触发 |
| Debugger | Memory | `mmu_read_32(mmu, pmem, vaddr, ...)` 显示内存内容 | 用户 `x` 命令 |
...
2. 遍历 Program Headers，对每个 PT_LOAD 段：
   Loader 调用 mem_map(&pmem, paddr, size, flags, "segment") 分配物理内存区域
   Loader 调用 mem_load(&pmem, paddr, data, size) 写入代码/数据
3. Loader 调用 mem_map(&pmem, 0x07EC0000, 0x40000, MEM_READ | MEM_WRITE, "stack") 建栈（恒等映射 128MB 内；完善版 0xBFFC0000~0xC0000000）
...
```
while (running) {
    ① Fetch:   Instr = mmu_read_32(mmu, pmem, PC, ...)    // CPU → MMU → PhysicalMemory
    ② Decode:  拆出 opcode / rd / rs1 / rs2
    ③ Execute: result = ALU(regs[rs1], regs[rs2])
    ④ Memory:  if (lw) regs[rd] = mmu_read_32(mmu, pmem, addr, ...)   // CPU → MMU → PhysicalMemory
               if (sw) mmu_write_32(mmu, pmem, addr, regs[rs2], ...)  // CPU → MMU → PhysicalMemory
...
- **Loader ↔ Memory**：Loader 通过 `mem_map` + `mem_load` 写入。Memory 负责校验地址合法性。
- **CPU ↔ Memory**：CPU 通过 `mmu_read_32` / `mmu_write_32` 访问。MMU 负责地址翻译和权限检查，访问越界或权限不对 → 返回 `false` + 填充 `ExceptionType`，CPU 调用 `cpu_trap()` 处理。
