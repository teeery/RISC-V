...
bool exec_fma(Simulator *sim, DecodedInstr *dec, uint32_t *next_pc)
{
    ...
    switch (dec->opcode) {
    case 0x43: res.f = fmaf(op_a.f, op_b.f, op_c.f);  break;     // FMADD.S:  a×b + c
    case 0x47: res.f = fmaf(op_a.f, op_b.f, -op_c.f); break;     // FMSUB.S:  a×b - c
    case 0x4B: res.f = fmaf(-op_a.f, op_b.f, op_c.f); break;     // FNMSUB.S: -a×b + c
    case 0x4F: res.f = -fmaf(op_a.f, op_b.f, op_c.f); break;     // FNMADD.S: -a×b - c
    default:
        cpu_trap(sim, EXC_ILLEGAL_INST, dec->opcode);
        return false;
    }
